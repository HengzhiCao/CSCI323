...
# T(n) = 8T(n/4) + n
def f16_midterm_fall_2008(func, n):
    if n == 1:
        return 1
    else:
...
# T(n) = 2T(n/2) + O(n)
def f20_slides_of_6(func, n):
    if n == 1:
        return 1
    else:
        return 2 * eval_func(func, int(n / 2)) + 0 * n
...
def main():
    ...
    call_and_print(f16_midterm_fall_2008, 256, "T(n) = 8T(n/4) + n,")
    call_and_print(f17_catalan_number, 100, "T(n) = sum i=0 to n of (T(i) * T(n-1-i))")
    call_and_print(f18_sum_of_squares, 100, "T(n) = T(n-1) + n^2")
    call_and_print(f19_sum_of_cubes, 100, "T(n) = T(n-1) + n^3")
    call_and_print(f20_slides_of_6, 256, "T(n) = 2T(n/2) + O(n)")
    print_dicts()
